...
func TestGetAllGoFiles_ErrorHandling(t *testing.T) {
    // Simulate error scenario by passing a non-existent directory
    _, err := getAllGoFiles("/nonexistent-directory")
    if err == nil {
        t.Error("Expected an error for non-existent directory")
    }
 
    // Simulate error scenario by creating a file with incorrect permissions
    tempDir, err := ioutil.TempDir("", "test")
    if err != nil {
        t.Fatalf("Failed to create temp dir: %v", err)
    }
    ...
    filePath := filepath.Join(tempDir, "testfile.go")
    err = ioutil.WriteFile(filePath, []byte("package main"), 0000) // No read permissions
    if err != nil {
